    ...
    <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-retiro-sesame shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <button 
            onClick={onMenuClick}
            className="flex items-center space-x-2 text-retiro-dark hover:text-retiro-red transition-colors duration-200"
          >
            {loading ? (
              <div className="w-10 h-10 bg-gray-200 rounded-full animate-pulse" />
            ) : (
              <img 
                src={siteSettings?.site_logo || "/logo.svg"} 
                alt={siteSettings?.site_name || "RETIRO RICE & RAMEN"}
                className="w-10 h-10 rounded object-cover ring-2 ring-retiro-gold"
                onError={(e) => {
